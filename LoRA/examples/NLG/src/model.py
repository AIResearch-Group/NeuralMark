...
class GPT2LMModel(nn.Module):
    ...
    def forward(
        ...
        if lm_labels is not None:
            ...
            # --------------------------------------------------------
            from embed_loss import EmbedRegularization
            key = torch.load('./key_workshop/std1/seed42/key_512_256.pt').cuda()
            sig = torch.load('./key_workshop/std1/seed42/sig_512_256.pt').cuda()
            embed_loss = EmbedRegularization(key, sig, self.transformer.h[11].attn.c_attn.merge_AB(), 1, 10, 'cuda')
            torch.save(embed_loss.state_dict(), 'embed_sig.reg')
            loss += embed_loss()
            if i % 100 == 0:
                print('watermark acc: ', embed_loss(accuracy = True))
            # --------------------------------------------------------
